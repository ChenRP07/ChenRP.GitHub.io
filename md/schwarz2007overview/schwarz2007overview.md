...
### System Interface
SVC的一个目标是支持简单的比特流操作。为了提取伸缩的子流应该从比特流中移除一部分不需要的NAL单元。因此每个NAL单元都需要具有空间、时间和质量标识符等参数，同时需要知道高层的层间预测需要哪些NAL单元。为了便于比特流操作，SVC针对H264的1字节头额外拓展了3个字节，分别为D、Q、T和优先级P。SVC引入了前缀NAL单元，也制定了额外的SEI消息。

### Bit Stream Switching
在每个access单元中均可以实现依赖层内的不同质量细化层的切换。但是要在依赖层之间进行切换必须从IDR access单元开始。IDR access单元标识着从当前单元及后继单元开始的解码独立于之前的access单元。
